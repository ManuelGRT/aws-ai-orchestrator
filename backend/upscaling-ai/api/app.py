...
def get_application(settings: APISettings) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        try:
            # intenta Vulkan (gpu id 0)
            # app.state.realesrgan = Realesrgan(gpuid=0, model=4)
            logger.info("Inicializando RealESRGAN en CPU (gpuid=-1)")
            app.state.realesrgan = Realesrgan(gpuid=-1, model=4, tilesize=64)
        except Exception as e:
            logger.warning("Vulkan init failed, falling back to CPU (gpuid=-1)", exc_info=e)
            # CPU fallback (sin Vulkan)
            app.state.realesrgan = Realesrgan(gpuid=-1, model=4, tilesize=64)
        yield
